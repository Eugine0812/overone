...
def welcome(message):
    bot.send_message(message.chat.id,
                       "Добро пожаловать, {0.first_name}!\nНиже Вы можете выбрать подходящий для Вас тур и ознакомиться с расписанием предстоящих туров." .format(
...
def lalala(message):
    if message.chat.type == 'private':
        if message.text == "Туры по Беларуси":
            bot.send_message(message.chat.id, 'Ближайшие туры выходного дня по РБ:\n\n-Байдарки + поход на болото + усадьба\nhttps://psixtravel.by/tour/splav-boloto/'
                                              '\n\n-Сплав по рекам Узлянка-Нарочанка\nhttps://psixtravel.by/tour/splav-uzljanka-narochanka/\n\n-Сплав! Бацька Нёман!\n'
            ...
            # photo = open("plan.png", 'rb')
            # bot.send_photo(message.from_user.id, photo)
            webbrowser.open ( 'https://docs.google.com/spreadsheets/d/e/2PACX-1vRc3rzf-0SY580lMQA2kidCHbr9_Eu-hctYd5EPIGY6DoYiQYliFiWrx-XhCjjq_CxwHtpBfHNNqLGd/pubhtml?gid=293021810&single=true', new=2 )
        else:
    ...
    try:
        if call.message:
            ...
            bot.edit_message_text ( chat_id=call.message.chat.id, message_id=call.message.message_id,
                                    text="Выберите  подходящий тур для Вас:",
                                    reply_markup=None )
